...
if ( abs(NextPose(3) - Pose(3))/time > 5)  % more than 5 m/s
    if NextPose(3)>Pose(3)
        NextPose(3) = 5*time + Pose(3);
    else
        NextPose(3) = -5*time + Pose(3);
    end
end
...
if ( abs(NextPose(1) - Pose(1))/time > 5)  % more than 5 m/s
    if NextPose(1)>Pose(1)
        NextPose(1) = 5*time + Pose(1);
    else
        NextPose(1) = -5*time + Pose(1);
    end
end

if ( abs(NextPose(2) - Pose(2))/time > 5)  % more than 5 m/s
    if NextPose(2)>Pose(2)
        NextPose(2) = 5*time + Pose(2);
    else
        NextPose(2) = -5*time + Pose(2);
    end
end
